...
Parents can self-register, sign in, and keep one delivery address plus contact numbers on their account. The shop has exactly one seeded admin who can sign in, change her password, and reset a parent’s password. A single-use admin recovery code is printed to the server log, never the UI. Both apps exist as real doors: Chalk & Brass chrome, light and dark, navigation shells, and auth gates. Identity, sessions, and shells only — catalog, cart, and orders stay empty structure.
...
**Parent.** Register with name, one delivery address, WhatsApp, email, and password; second phone optional. Login is email + password only — WhatsApp is contact, never an identity. Email is the sole login identity, not verified in v1, visible on Account and not editable. Profile can change name, address, WhatsApp, and second phone (future orders only). One address — no address book, no per-order override. Password change while logged in is in-app. Own profile only.
...
Stack: Node 22.12+ (`engines.node` `>=22.12.0`; Bonto runtime Node 22, not default 20); Express 5.2.1, better-sqlite3 13.0.3, TypeScript 7.0.2, React 19.2.8, Vite 8.2.1, tsx 4.23.12. No Next.js, MUI/shadcn/Tailwind, ORM, JWT, or tRPC. Dev: tsx + Vite proxying `/api`. Prod: `tsc` + `vite build`; Express serves the built apps.
...
SQL `snake_case`; JSON `camelCase`. Time UTC in DB, Asia/Colombo on screens. Money, when shown: integer rupees in data, `Rs.` prefix with comma thousands and no decimals on screen. EXPERIENCE wins over DESIGN on behaviour.
...
**Auth gate.** Offers Log in and Create account. After login/register from Cart, Orders, or Account — or after a timed-out session — land on Browse, not the tab that was tapped. Do not auto-add anything. No parent forgot-password screen. Announce the gate to assistive tech. Add-to-cart resume-to-pack is a later epic.

**Forms and shell.** Label-caps above a raised control. Error: 3px danger border and message beneath; keep the typed value. Focus is the doubled ring on `:focus-visible`. One primary per surface. Voice: warm, plain-spoken shopkeeper; every refusal says what to do next. Cold-start: branded skeleton in the shape of that page (dashed rows, accent-quiet bars), no spinner, no wording; no shimmer under reduced motion. Later loads: spinner only. Unknown URL: empty page — no message, refresh, or back CTA.

## Cross-Story Dependencies

- Scaffold and shell precede every other story in this epic.
- Admin seed, session cookie, and recovery-code API precede parent login and admin password work.
- Parent accounts and storefront session precede parent Account and admin-set parent password.
- Catalog, cart, and orders folders are empty structure only. Pack content is the next epic. Cart and Add-to-cart auth-resume wait until after catalog.
